...
os.environ.setdefault("COQUI_TOS_AGREED", "1")  # ✅ evita prompt CPML si aplica
...
# ----------------------------
# ✅ Detectar repo real de MuseTalk
# ----------------------------
def _detect_musetalk_root(base: str) -> str:
    # 1) si el user lo setea explícito, respetarlo (pero lo validamos)
    env_root = (os.environ.get("MUSE_ROOT") or "").strip()
    if env_root:
        if os.path.isfile(os.path.join(env_root, "scripts", "inference.py")):
            return env_root
        if os.path.isfile(os.path.join(env_root, "inference_config.json")):
            return env_root

    # 2) candidatos típicos en tu volumen
    candidates = [
        os.path.join(base, "MuseTalk"),
        os.path.join(base, "musetalk"),
        os.path.join(base, "musetalk_ok"),   # OJO: a veces es venv, por eso validamos por scripts/
        os.path.join(base, "projects", "MuseTalk"),
        os.path.join(base, "projects", "musetalk"),
    ]

    for c in candidates:
        if os.path.isfile(os.path.join(c, "scripts", "inference.py")):
            return c

    # 3) fallback: buscar en 2 niveles (sin llenar disco ni nada)
    #    solo para encontrar "scripts/inference.py"
    for c in [base, os.path.join(base, "projects")]:
    ...
    # si no aparece, error claro
    raise RuntimeError(
        "No encuentro el repo de MuseTalk (falta scripts/inference.py). "
        "Asegúrate que el repo exista en el volumen (ej: /runpod-volume/MuseTalk)."
    )

MUSE_ROOT = _detect_musetalk_root(BASE)
...
def _tts_make_wav(text: str, voice: str, lang: str, out_wav: str):
    ...
    # ✅ por si Coqui intenta preguntar (aunque COQUI_TOS_AGREED=1 ya ayuda)
...
def _musetalk_infer(input_mp4: str, audio_wav: str) -> str:
    ...
    infer_py = os.path.join(MUSE_ROOT, "scripts", "inference.py")
    _require_file(infer_py, "MuseTalk scripts/inference.py")
        ...
        if mode in ("echo", "debug"):
            return {
                ...
                "checks": {
                    ...
                    "inference_py_exists": os.path.isfile(os.path.join(MUSE_ROOT, "scripts", "inference.py")),
